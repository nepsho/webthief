...
function checkWildCard(fields) {
  ...
  response.skip = false;
  if (bvalid.isString(fields[0]) && fields[0].trim() == '*') {
...
exports.getWebsiteDetails = function (_url, _html, _option) {
  return new Promise(function (resolve, reject) {
    ...
    var metaData = MetaExtractor.returnMetaData(
      _html,
      getOptionFields(_option),
      hostname,
    );
    ...
    resolve({
      success: true,
      response: nuliFyEmptyFields(copy.deepcopy(metaData)),
    });
  });
};
