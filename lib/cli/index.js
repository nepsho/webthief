...
if (bvalid.isString(_f) && !bvalid.isString(_i)) {
  ...
  invalid_input_err += color.bold(
    'No any option url found, please enter url first',
  );
  invalid_input_err += '\n';
  process.stdout.write(color.fg(invalid_input_err, [255, 255, 0]));
  process.exit(1);
...
function showErrorOutput(obj) {
  ...
  errorMsg += '\n';
  process.stdout.write(color.inv(color.fg(errorMsg, [255, 0, 0])));
  try {
    if (bvalid.isObject(obj)) {
      var keys = Object.keys(obj);
      var opLine;
...
if (valid_f.indexOf(_f) < 0) {
  ...
  invalid_mode_err += '\n';
  process.stdout.write(color.fg(invalid_mode_err, [255, 0, 0]));
  process.exit(1);
}
    ...
    .then(function (data) {
      if (bvalid.isObject(data) && bvalid.isArray(data.response)) {
        for (var i = 0; i < data.response.length; i++) {
          ...
          var op =
            '(' + index + ')' + space + color.fg(data.response[i], [0, 255, 0]);
          console.log(op);
        }
        if (bvalid.isString(_o) && _o.trim() == '-d') {
          ...
          download.downloadImages(_i, data.response, dir, function (response) {
            ...
            if (response.success) {
              console.log(
                color.fg(color.fg(color.bold(response.message), [0, 255, 0])),
              );
            } else {
              console.log(
                color.bold(color.fg(color.bold(response.message), [255, 0, 0])),
              );
            }
          });
        }
        if (data.response.length == 0) {
          ...
          no_img_err += '\n';
          process.stdout.write(color.fg(no_img_err, [255, 0, 0]));
        }
      }
    })
    .catch(function (error) {
